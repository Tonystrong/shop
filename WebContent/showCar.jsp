...
<head>
	...
	<script type="text/javascript">
		$(function() {
			$("#del").click(function() {
				if(confirm('你确定要删除吗？')) {
					$.get('sorderAction_delete.action');
				} 		
			...
			$(".text").change(function() {
				...
				if (!isNaN(number) && number>0 && parseInt(number)==number) {
					var pid = $(this).parents("tr:first").attr("lang");
					//将pid和修改的值传回业务修改
					$.post("sorderAction_update.action", {number:number, 'product.id':pid}, function(total) {
						//利用id选择器修改
						$("#total").html('￥'+total);
						$("#ptotal").html('￥'+total);
					}, "text");
				...
				<div class="totals">
						...
						<tbody>
							<tr>
								...
								<td class="align_right" style=""><strong>
									<span id="ptotal" class="price">￥${forder.total}</span>
								</strong>
								</td>
							</tr>
							<tr>
							...
							<tr>
								...
								<td class="align_right" style=""><span id="total" class="total"><strong>￥${forder.total}</strong>
								</span>
								</td>
							</tr>
						</tbody>
					...
					<div class="action_buttonbar">
						<button type="button" title="" class="checkout fr"
							...
							<font><a href="${shop}/user/confirm.jsp">确认订单</a>
							</font>
						</button>
						<button type="button" title="" class=" fr" id="del">
							<font><a href="#" class="delete">清空购物车</a>
							</font>
						</button>
